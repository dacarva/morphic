...
export async function createFheInstance(
  ...
  const failPermit = await instance.generatePermit(
    contractAddress,
    provider,
    signer[1],
  );
  const failPermission = instance.extractPermitPermission(failPermit);

  return Promise.all([instance, permission, failPermission]).then(
    ([instance, permission, failPermission]) => ({
      instance,
      permission,
      failPermission,
    }),
